  ...
  interface TableProps {
    ...
    onGenerateAll: () => void
    onSendSelected: () => void
    onGenerateAndSend: () => void // Новая функция для комбинированного действия
    loading?: string
    generatingId?: string
  }
  ...
  export default function VacanciesTable({
    ...
    const isGenerating = loading === 'generate' || !!generatingId
    ...
    if (isMobile) {
      return (
        <>
          {/* Мобильный заголовок с выделить все */}
          {vacancies.length > 0 && (
          ...
          {/* Якорная ссылка наверх для мобильной версии */}
          {vacancies.length > 0 && (
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#e7e7e7] p-3">
            <div className="flex gap-2 mb-2">
              <LoadingButton
                ...
                disabled={isGenerating || isGeneratingAndSending || selectedCount === 0}
            ...
            <LoadingButton
              ...
              disabled={isGeneratingAndSending || isGenerating || isSending || selectedNotSended === 0}
    ...
    return (
      <>
        ...
        {/* Якорная ссылка наверх для десктопной версии */}
        {vacancies.length > 0 && (
