...
export const CreditsInfo = ({
    ...
    const startPaymentStatusCheck = (paymentId: string, paymentWindow: Window | null) => {
        ...
        const checkInterval = setInterval(async () => {
            ...
            if (paymentWindow && paymentWindow.closed) {
                setTimeout(async () => {
                    if (onCreditsChange) await onCreditsChange()
    ...
    const renderPackage = (pkg: any, index: number, isMobile: boolean = false) => {
        const pricePerToken = (pkg.amount / pkg.credits).toFixed(1)
        const isSelected = selectedPackageId === pkg.id
        
        const showBadge = pkg.popular
        const badgeText = 'ТОП'
        ...
        return (
            <div
                ...
                className={`border rounded-lg ${isMobile ? 'p-3' : 'p-4'} cursor-pointer transition-all relative ${
                    isSelected ? 'ring-2 ring-[#d6001c] bg-red-50 border-[#d6001c]' : 'hover:shadow-md border-[#e7e7e7] active:bg-gray-50'
                }`}
            >
                <div className={`flex ${isMobile ? 'items-center justify-between' : 'items-start justify-between'}`}>
                    <div className="flex-1">
                        <div className={`font-bold ${isMobile ? 'text-base' : 'text-xl'} text-[#232529] flex items-center`}>
                            {pkg.credits} токенов
                            {/* АДАПТИРОВАННЫЙ БЛОК БЭЙДЖА: ВСТРОЕННЫЙ */}
                            {showBadge && (
                                <span className="ml-2 px-2 py-0.5 rounded-full bg-[#d6001c] text-white font-medium text-xs uppercase">
                                    {badgeText}
                                </span>
                            )}
                            {/* КОНЕЦ АДАПТИРОВАННОГО БЛОКА */}
                        </div>
                        <div className={`${isMobile ? 'text-sm mt-0.5' : 'text-base mt-1'} text-[#999999]`}>
                            {pkg.amount.toLocaleString('ru-RU')} ₽
                        </div>
                        <div className={`${isMobile ? 'text-xs mt-1' : 'text-sm mt-2'} text-[#666666] font-medium`}>
                            {pricePerToken} ₽ за отклик
                        </div>
                    </div>
                    {isSelected && (
                        <svg className={`${isMobile ? 'w-5 h-5 ml-3' : 'w-6 h-6 ml-2'} text-[#d6001c] flex-shrink-0`} fill="currentColor" viewBox="0 0 20 20">
    ...
    return (
        <div id="credits-section" className={`hh-card p-4 md:p-6 mb-4 ${(!hasCredits || needTokens) ? 'border-orange-300 bg-orange-50' : ''}`}>
            <div>
                ...
                <div className="text-sm md:text-base text-[#666666] md:ml-3 mb-4">
                    Откликов за сегодня <span className="font-bold text-[#232529]">{applications24h}/200</span> (лимит headhunter)
                </div>
            </div>
            ...
            <div className="md:hidden">
                ...
                >
                    ...
                    <svg className={`w-5 h-5 text-[#999999] transition-transform ${showPackages ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {showPackages && (
                    <div className="pt-4 pb-2 border-t border-[#e7e7e7] -mx-4 px-4">
                        <div className="space-y-3">
                            {packages.map((pkg, idx) => renderPackage(pkg, idx, true))}
                        </div>
                        <LoadingButton
                        ...
                        >
                            {selectedPackage ? `Оплатить ${selectedPackage.amount.toLocaleString('ru-RU')} ₽` : 'Выберите пакет'}
                        </LoadingButton>
                        <div className="text-xs text-[#999999] text-center mt-3">
                            <a href="/offerta">Публичная офферта</a> • <a href="https://t.me/reetry" target="_blank" rel="noopener noreferrer">Поддержка(Telegram)</a>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="hidden md:block">
                ...
                >
                    {selectedPackage ? `Купить за ${selectedPackage.amount.toLocaleString('ru-RU')} ₽` : 'Выберите пакет'}
                </LoadingButton>
